...
def test_recalculate_all_applies_engineer_workbook_crusher_design_rate_and_power():
    ...
    # Crusher design rate = mill DESIGN rate × availability ratio (template B22):
    # 1596 × 1.15 × (92/75) = 2251.4 t/h (mill 15% factor, not a 25% crushing factor).
    assert cur.updates["r1"] == pytest.approx(2251.4, abs=0.1)
    assert cur.updates["r2"] == pytest.approx(528000 / 135000, abs=0.001)
    assert cur.updates["r3"] == pytest.approx(0.2385, abs=0.001)
    assert cur.updates["r4"] == pytest.approx(537.1, abs=0.3)
