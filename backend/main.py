...
def _fix_crusher_design_nominal_safe() -> None:
    """Recompute design_value and nominal_value for GIRATOIRE 'Débit design alimentation'.

    Runs in its own connection+transaction, isolated from other startup fixes.
    Root cause: dag_key='target_tph' caused the cascade to overwrite design_value
    ...
    try:
        ...
        c.rollback()
        logger.exception("crusher design fix failed (non-fatal)")
    finally:
        if cur is not None:
            cur.close()
        release(c)
